...
const chars = "QWERTYUIOPÅASDFGHJKLÖÄZXCVBNM".split("");
...
const GuessChar = () => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-6 md:grid-cols-11 gap-1 w-max mx-auto text-center">
        {chars.map((c) => (
          <div key={c}>
            {guessed.includes(c) ? (
              <button className="text-4xl border-4 h-20 w-20 max-[1900px]:border-2 max-[1900px]:h-14 max-[1900px]:w-14 text-green-500 border-green-500 rounded-md hover:bg-slate-300">
                {c}
              </button>
            ) : guessedWrong.includes(c) ? (
              <button className="text-4xl border-4 h-20 w-20 max-[1900px]:border-2 max-[1900px]:h-14 max-[1900px]:w-14 text-red-600 border-red-600 rounded-md hover:bg-slate-300">
                {c}
              </button>
            ) : guessed.length == distinctCharArray.length ? (
              <button className="text-4xl border-4 h-20 w-20 max-[1900px]:border-2 max-[1900px]:h-14 max-[1900px]:w-14 border-black rounded-md bg-slate-300">
                {c}
              </button>
            ) : GuessAmount > 0 ? (
              <button
        ...
        {guessed.length == distinctCharArray.length ? (
          <div>
            <div className="text-center">
              <p className="font-medium mt-10 max-[1900px]:mt-6 text-xl">
                VOITIT PELIN!
              </p>
              <button
                className="border-4 border-sky-300 p-4 mt-4 rounded-xl bg-sky-300 hover:border-sky-400"
          ...
          <div className="text-center">
            <p className="font-medium mt-10 max-[1900px]:mt-6 text-xl">
              VÄÄRIÄ ARVAUKSIA JÄLJELLÄ: {GuessAmount}
            </p>
            <button
              className="border-4 border-sky-300 p-4 mt-4 rounded-xl bg-sky-300 hover:border-sky-400"
              onClick={() => window.location.reload()}
